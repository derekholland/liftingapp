...
const WorkoutForm = () => {
	...
	const handleSubmit = async (e: React.FormEvent) => {
		...
		try {
			...
			// After successfully adding a workout, reload the page to reflect the changes
			window.location.reload(); // Full page reload to ensure the main page reflects the new workout
		} catch (error: unknown) {
			if (error instanceof Error) {
				alert(error.message);
			} else {
